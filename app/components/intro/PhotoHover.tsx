	...
	return (
		<>
			...
			{previewMode && createPortal(
				<figure
					aria-hidden="true"
					className={`pointer-events-none fixed z-50 rounded-lg bg-white p-1 shadow-xl md:p-2 ${isMobilePreview ? 'left-1/2 -translate-x-1/2' : '-translate-x-full'}`}
					style={isMobilePreview ? { top: cursorPosition.y } : { left: cursorPosition.x - 16, top: cursorPosition.y + 16 }}
				>
					<Image alt="" className="h-auto w-160 max-w-[70vw] md:w-100 rounded-lg" height={5152} loading="eager" src="/other-images/half-dome.JPG" width={6864} />
					<figcaption className="mt-2 text-xs leading-5 text-stone-600" style={{ fontFamily: 'Consolas, ui-monospace, monospace' }}>
						...
						<span className="block md:hidden">Yosemite National Park · 2026</span>
