...
export default function Header() {
  ...
  return (
    <header className="z-[999] relative transition-transform">
      ...
      <motion.div
        className="block sm:hidden fixed top-0 left-1/2 h-[4.5rem] w-full rounded-none border border-white border-opacity-40 bg-white shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] dark:bg-gray-950 dark:border-black/40"
        initial={{ y: -100, x: "-50%", opacity: 0 }}
        animate={{ y: 0, x: "-50%", opacity: 1 }}
      ></motion.div>

      <nav className="flex fixed top-[0.7rem] sm:bottom-auto left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0 mx-auto px-2">
            ...
            >
              ...
              >
                ...
                {link.name === activeSection && (
                  <motion.span
                    className="bg-gray-400/20 rounded-2xl sm:rounded-full absolute inset-0 -z-10 dark:bg-gray-800 px-2 py-6 sm:py-0 sm:px-0 top-[-.7rem] sm:top-0"
